...
def test_get_production_version_returns_none_when_no_production(mock_client):
...
def test_get_production_version_returns_none_when_model_missing(mock_client):
    # Simulate "model not registered" — MLflow raises a RestException.
...
def test_get_production_mape_reads_tag(mock_client):
    mock_client.get_latest_versions.return_value = [
        _make_version("2", tags={"production_mape": "3.5"})
    ]
...
def test_get_production_mape_returns_none_when_tag_missing(mock_client):
    mock_client.get_latest_versions.return_value = [_make_version("2", tags={})]
...
def test_should_promote_true_when_new_mape_is_better(mock_client):
    mock_client.get_latest_versions.return_value = [
        _make_version("2", tags={"production_mape": "5.0"})
    ]
...
def test_should_promote_false_when_new_mape_is_worse(mock_client):
    mock_client.get_latest_versions.return_value = [
        _make_version("2", tags={"production_mape": "3.0"})
    ]
...
def test_promote_to_production_transitions_and_tags(mock_client):
